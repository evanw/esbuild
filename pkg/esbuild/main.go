...
func exitWithError(text string) error {
	colorRed := ""
	colorBold := ""
	colorReset := ""
	...
	return exitcode.Set(errors.New(text), 1)
}
...
// CLI expects to receive os.Args[1:] or testing equivalent.
func CLI(osargs []string) error {
	...
	if len(args.entryPaths) == 0 {
		if len(os.Args) < 2 {
			...
			return exitWithError("No input files")
		}
	}
	...
	err = func() error {
		...
		if args.traceFile != "" {
			...
			if err != nil {
				return exitWithError(fmt.Sprintf("Failed to create a file called '%s': %s", args.traceFile, err.Error()))
			}
			defer func() {
				f.Close()
		...
		if args.cpuprofileFile != "" {
			...
			if err != nil {
				return exitWithError(fmt.Sprintf("Failed to create a file called '%s': %s", args.cpuprofileFile, err.Error()))
			}
			defer func() {
				f.Close()
