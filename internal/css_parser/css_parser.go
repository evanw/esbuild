			...
			if opts.composesContext != nil && len(list) == 1 && len(list[0].Selectors) == 1 && list[0].Selectors[0].IsOnlyAmpersand() {
				...
				for _, sel := range list {
					...
					if first.Combinator.Byte != 0 {
						...
						for _, nestingSelectorLoc := range first.NestingSelectorLocs {
							composesContext.problemRange = logger.Range{Loc: logger.Loc{Start: int32(nestingSelectorLoc.GetIndex())}, Len: 1}
						}
					} else {
