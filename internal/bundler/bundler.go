			...
			for importRecordIndex := range records {
				...
				if record.GlobPattern != nil {
					...
					case ast.ImportNewURL:
						prettyPath = fmt.Sprintf("new URL(%q, import.meta.url)", prettyPath)
					}
		...
		go func(i int, entryPoint EntryPoint) {
			...
			if resolveResult != nil {
				...
				if !s.fs.IsAbs(entryPoint.InputPath) {
					if query := s.res.ProbeResolvePackageAsRelative(entryPointAbsResolveDir, entryPoint.InputPath, ast.ImportEntryPoint); query != nil {
						notes = append(notes, logger.MsgData{
	...
	for _, key := range keys {
		...
		if !resolveResult.IsExternal {
			sourceIndex = ast.MakeIndex32(s.maybeParseFile(
				resolveResult,
				s.res.PrettyPath(resolveResult.PathPair.Primary),
				importSource,
				importRange,
				inputKindNormal,
				nil,
		...
		case ast.ImportNewURL:
			value.Data = &js_ast.ENewURLImportMeta{ImportRecordIndex: importRecordIndex}
		default:
