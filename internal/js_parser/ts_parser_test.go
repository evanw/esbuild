...
func TestTSTypes(t *testing.T) {
	...
	expectPrintedTS(t, "type Foo<in T> = keyof T;", "")
	expectPrintedTS(t, "type Foo<out T, out K extends keyof T> = T[K];", "")
	expectPrintedTS(t, "type Foo<in out T> = T[keyof T];", "")
	expectPrintedTS(t, "type Foo<in T> = keyof T;", "")
	...
	expectPrintedTSX(t, "<out T,>() => {}", "() => {\n};\n")
	expectPrintedTSX(t, "<in out T,>() => {}", "() => {\n};\n")
	expectPrintedTSX(t, "<in T extends any>() => {}", "() => {\n};\n")
	expectPrintedTSX(t, "<out T extends any>() => {}", "() => {\n};\n")
	expectPrintedTSX(t, "<in out T extends any>() => {}", "() => {\n};\n")
	expectParseErrorTS(t, "type Foo<public T> = T;", "<stdin>: ERROR: Expected \">\" but found \"T\"\n")
	expectParseErrorTS(t, "type Foo<in out in T> = T;", "<stdin>: ERROR: Expected \">\" but found \"in\"\n")
	expectParseErrorTS(t, "type Foo<in out out T> = T;", "<stdin>: ERROR: Expected \">\" but found \"T\"\n")
	expectParseErrorTS(t, "type Foo<out in T> = T;", "<stdin>: ERROR: Expected \">\" but found \"in\"\n")
