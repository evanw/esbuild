...
func TestTSTypes(t *testing.T) {
	...
	// TypeScript 4.7
	expectPrintedTS(t, "type Covariant<out T> = { x: T; }", "")
	expectPrintedTS(t, "type Contravariant<in T> = { f: (x: T) => void; }", "")
	expectPrintedTS(t, "type Invariant<in out T> = { f: (x: T) => T; }", "")
	expectPrintedTS(t, "type T10<out T> = T;", "")
	expectPrintedTS(t, "type T11<in T> = keyof T;", "")
	expectPrintedTS(t, "type T12<out T, out K extends keyof T> = T[K];", "")
	expectPrintedTS(t, "type T13<in out T> = T[keyof T];", "")
	expectPrintedTS(t, "type Foo3<in out T> = { x: T; f: FooFn3<T>; }", "")
	expectParseErrorTS(t, "type T20<public T> = T;", "<stdin>: ERROR: Expected \">\" but found \"T\"\n")
	expectParseErrorTS(t, "type T21<in out in T> = T;", "<stdin>: ERROR: Expected \">\" but found \"in\"\n")
	expectParseErrorTS(t, "type T22<in out out T> = T;", "<stdin>: ERROR: Expected \">\" but found \"T\"\n")
	expectParseErrorTS(t, "type T23<out in T> = T;", "<stdin>: ERROR: Expected \">\" but found \"in\"\n")
	...
	expectPrintedTS(t, "declare class StateNode<TContext, in out TEvent extends { type: string }> { _storedEvent: TEvent; _action: ActionObject<TEvent>; _state: StateNode<TContext, any>; }", "")
	...
	expectParseErrorTSX(t, "<in T extends any>() => {}", "<stdin>: ERROR: The character \">\" is not valid inside a JSX element\nNOTE: Did you mean to escape it as \"{'>'}\" instead?\n<stdin>: ERROR: Unexpected end of file\n")
	expectParseErrorTSX(t, "<out T extends any>() => {}", "<stdin>: ERROR: The character \">\" is not valid inside a JSX element\nNOTE: Did you mean to escape it as \"{'>'}\" instead?\n<stdin>: ERROR: Unexpected end of file\n")
	expectParseErrorTSX(t, "<in out T extends any>() => {}", "<stdin>: ERROR: The character \">\" is not valid inside a JSX element\nNOTE: Did you mean to escape it as \"{'>'}\" instead?\n<stdin>: ERROR: Unexpected end of file\n")
}
...
func TestTSPrivateIdentifiers(t *testing.T) {
	...
	expectParseErrorTS(t, "class Foo { @dec #foo() {} }", "<stdin>: ERROR: Expected identifier but found \"#foo\"\n")
	expectParseErrorTS(t, "class Foo { @dec get #foo() {} }", "<stdin>: ERROR: Expected identifier but found \"#foo\"\n")
	expectParseErrorTS(t, "class Foo { @dec set #foo() {x} }", "<stdin>: ERROR: Expected identifier but found \"#foo\"\n")
