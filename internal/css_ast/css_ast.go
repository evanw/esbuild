...
type CompoundSelector struct {
	...
	NestingSelectorLocs []ast.Index32 // List of "&"
...
func (sel *CompoundSelector) HasNestingSelector() bool {
	// Assume that all locations have same status
...
func (sel CompoundSelector) Range() (r logger.Range) {
	...
	for _, nestingSelectorLoc := range sel.NestingSelectorLocs {
		if nestingSelectorLoc.IsValid() {
			r.ExpandBy(logger.Range{Loc: logger.Loc{Start: int32(nestingSelectorLoc.GetIndex())}, Len: 1})
		}
	}
