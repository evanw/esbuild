...
let isInternalWorkerThread =
  worker_threads?.workerData?.esbuildVersion === ESBUILD_VERSION;
...
let esbuildCommandAndArgs = (): [string, string[]] => {
  ...
  if (
    (!ESBUILD_BINARY_PATH || WASM) &&
    (path.basename(__filename) !== "main.js" ||
...
export let transform: typeof types.transform = (
...
export let analyzeMetafile: typeof types.analyzeMetafile = (
  messages,
  options,
) => ensureServiceIsRunning().analyzeMetafile(messages, options);

export let buildSync: typeof types.buildSync = (
...
export let transformSync: typeof types.transformSync = (
  input: string | Uint8Array,
  options?: types.TransformOptions,
) => {
  // Try using a long-lived worker thread to avoid repeated start-up overhead
...
export let formatMessagesSync: typeof types.formatMessagesSync = (
  messages,
  options,
) => {
  // Try using a long-lived worker thread to avoid repeated start-up overhead
...
export let analyzeMetafileSync: typeof types.analyzeMetafileSync = (
  metafile,
  options,
) => {
  // Try using a long-lived worker thread to avoid repeated start-up overhead
...
let ensureServiceIsRunning = (): Service => {
  ...
  let child = child_process.spawn(
    command,
    args.concat(`--service=${ESBUILD_VERSION}`, "--ping"),
    {
      windowsHide: true,
...
let runServiceSync = (
  callback: (service: common.StreamService) => void,
): void => {
  let [command, args] = esbuildCommandAndArgs();
  ...
  let stdout = child_process.execFileSync(
    command,
    args.concat(`--service=${ESBUILD_VERSION}`),
    {
      cwd: defaultWD,
      windowsHide: true,
      input: stdin,
...
let startWorkerThreadService = (
  ...
  let validateBuildSyncOptions = (
    options: types.BuildOptions | undefined,
  ): void => {
    if (!options) return;
  ...
  let runCallSync = (command: string, args: any[]): any => {
    ...
    } = worker_threads!.receiveMessageOnPort(mainPort)!;
    if (id !== id2)
...
let startSyncServiceWorker = () => {
  let workerPort: import("worker_threads").MessagePort =
    worker_threads!.workerData.workerPort;
